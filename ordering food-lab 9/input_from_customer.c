...
customer createCustomer() {
    ...
    c.password=(char*)malloc(MAX_PASSWD_LENGHT*sizeof(char));
    c.addInfo=(char*)malloc(MAX_ADDITIONAL_INFO*sizeof(char));
    return c;
}
void signUp(FILE *loginFile, customer *c, int *state, int *goOn ) {
    ...
    int cur_position=ftell(loginFile)+1;//save position of the file_pointer, to know where to return later
    int noOfUsers;//read no of users
    ...
    int conditionUsername=validateUsername(loginFile, noOfUsers, c->username, matchingPassword);
    decrypt_pass(key,matchingPassword);//decrypt password
    if(!conditionUsername) {
        printf("---Password:\n>");
        gets(c->password);
        int conditionPassword = validatePassword(c, matchingPassword);
        if(conditionPassword==2) {
            encrypt_pass(key,c->password);//encrypt password
            noOfUsers++;
            fseek(loginFile,cur_position,SEEK_SET);
            fprintf(loginFile,"%d",noOfUsers);
            fseek(loginFile,0,SEEK_END);
            fprintf(loginFile,"\r%s %s",c->username,c->password);
            (*state)++;//increments the state
            *goOn=0;//comes out of the while loop
        }
        else
            incorrectNewPassword(c);
    }
    else
        printf("%s\n",DUPLICATE_USER);
...
void login(customer *custom, int *state) {
    FILE *loginFile;
    displaySignOptions();
    ...
    *custom=createCustomer();
    switch (signChoice) {
        case 'a': {
            int goOn = 1;
            while (goOn)
                signIn(loginFile,custom, state, &goOn);
            break;
        }
        case 'b':{
            int goOn = 1;
            while (goOn)
                signUp(loginFile,custom, state, &goOn);
            break;
        }
        default: break;
    }
...
int validatePassword(customer *c, char matchingPassword[]) {
   if(strcmp(matchingPassword,c->password)==0)
       return 1; //matching password
    else if(isLongEnough(c->password) && notContainUsername(c) && containSpecialCharacters(c->password) && containDigits(c->password))
...
int notContainUsername(customer *c) {
    if(!strstr(c->password,c->username))
        return 1;
    return 0;
}
...
void incorrectNewPassword(customer *c) {
    if(!isLongEnough(c->password)) printf("%s\n",ERROR_PASSWORD_LONG);
    if(!notContainUsername(c)) printf("%s\n",ERROR_PASSWORD_NOT_USERNAME);
    if(!containSpecialCharacters(c->password)) printf("%s\n",ERROR_PASSWORD_SPECIAL_CHAR);
    if(!containDigits(c->password)) printf("%s\n",ERROR_PASSWORD_DIGITS);
}
